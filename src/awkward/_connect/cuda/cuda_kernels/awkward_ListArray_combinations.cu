...
//     # Fill parents as a run-length expansion of list indices:
//     if total > 0:
//         indices = cupy.arange(total, dtype=cupy.int64)
//         scan_in_array_parents = cupy.searchsorted(scan_in_array_offsets[1:], indices, side='right').astype(cupy.int64)
//     else:
